...
const videosLink = "http://lanapirsl.com/wordpress/wp-json/wp/v2/video";
...
function handleVideos(data){
    const myVideos=data;
    myVideos.forEach(showVideos);
}
function showVideos(singleObject){
    const template=document.querySelector("#videosTemplate").content;
    const copy = template.cloneNode(true);
    copy.querySelector("#videoPlayer").src=singleObject.ytlink;
    console.log(singleObject.ytlink);
    document.querySelector("#appendVideo").appendChild(copy);
}